...
export const getBranchesTool = {
  ...
  handler: async ({ repoPath, all = false }: { repoPath: string; all?: boolean }) => {
    try {
      ...
      const branches = {
        ...
        local: branchSummary.branches
          .filter(branch => !branch.name.includes('remotes/'))
          .map(branch => ({
            name: branch.name,
            commit: branch.commit,
            label: branch.label
          })),
        remote: all ? branchSummary.branches
          .filter(branch => branch.name.includes('remotes/'))
          .map(branch => ({
      ...
      logger.info(`Retrieved branches from ${repoPath}: ${branches.local.length} local, ${branches.remote.length} remote`);
