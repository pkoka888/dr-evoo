...
export const DEFAULT_SECURITY_CONFIG: SecurityConfig = {
  ...
  allowedFileExtensions: ['.txt', '.md', '.json', '.js', '.ts', '.py', '.html', '.css', '.xml', '.yaml', '.yml'],
 ...
 */
export function validatePath(filePath: string, config: SecurityConfig = DEFAULT_SECURITY_CONFIG): string {
 ...
 */
export function validateFileSize(filePath: string, config: SecurityConfig = DEFAULT_SECURITY_CONFIG): void {
  try {
    ...
    throw new SecurityError(`Cannot access file: ${error.message}`, 'FILE_ACCESS_ERROR');
  }
}
